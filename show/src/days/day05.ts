...
class Part1Solution implements Solution<Part1TraceItem> {
    ...
    solve(): Trace<Part1TraceItem> {
        ...
        for (let index = 0; index < updates.length; index++) {
            ...
            let isSafe = true;
            for (let pageIndex = 0; pageIndex < update.length - 1; pageIndex++) {
...
// For part2, if the check fails, we need to swap the items in the right order
type Part2TraceItemSwap = { kind: "items-swap", index: number, indexLhs: number, indexRhs: number };

type Part2TraceItem = Part1TraceItemInput | Part1TraceItemIndex | Part1TraceItemIndexOut | Part1TraceItemCheckSelect | Part1TraceItemCheckSelectOut | Part1TraceItemCheckSelectAgainst | Part1TraceItemCheckSelectAgainstOut | Part1TraceItemCheckSelectRuleBad | Part1TraceItemCheckSelectRuleGood | Part1TraceItemCheckSelectRuleOut | Part1TraceItemCheckSafe | Part1TraceItemTotal | Part2TraceItemSwap;
...
class Part2Solution implements Solution<Part2TraceItem> {
    ...
    solve(): Trace<Part2TraceItem> {
        ...
        for (let index = 0; index < updates.length; index++) {
            ...
            for (let pageIndex = 0; pageIndex < update.length - 1;) {
                trace.push({ kind: "check-select", index, pageIndex });
                const page = update[pageIndex];
                ...
                for (let pageIndex2 = pageIndex + 1; pageIndex2 < update.length; pageIndex2++) {
                    ...
                    const ruleIndex = this.hasRule(before, page2, page);
                    // If the rule is bad we need to swap page and page2
                    // then we need to start over this for loop
                    if (ruleIndex !== undefined) {
                        isSafe = false;
                        trace.push({ kind: "check-select-rule-bad", ruleIndex });

                        // Swap the items in the right order
                        trace.push({ kind: "items-swap", index, indexLhs: pageIndex, indexRhs: pageIndex2 });
                        const aux = update[pageIndex];
                        update[pageIndex] = update[pageIndex2];
                        update[pageIndex2] = aux;
                    } else {
                        trace.push({ kind: "check-select-rule-good" });
                    }
            ...
            trace.push({ kind: "index-out", index });
            trace.push({ kind: "check-safe", index, isSafe: wasSwapped });
...
class Part2Animator implements PartAnimator<Part2TraceItem> {
    ...
    private checkSelectRuleBad(step: Part1TraceItemCheckSelectRuleBad): number {
        ...
        this.orderItem!.classList.remove("text-gray-500", "text-green-500");
        this.orderItem!.classList.add("text-red-500");
    ...
    private checkSelectRuleGood(step: Part1TraceItemCheckSelectRuleGood): number {
        this.orderItem!.classList.remove("text-gray-500", "text-red-500");
        this.orderItem!.classList.add("text-green-500");
    ...
    private itemsSwap(step: Part2TraceItemSwap): number {
        const page = this.pages![step.index];
        const aux = page[step.indexLhs].text.textContent!;
        page[step.indexLhs].text.textContent = page[step.indexRhs].text.textContent!;
        page[step.indexRhs].text.textContent = aux;
